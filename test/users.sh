...
resp=$(curl -Lik "$ptcl://$host:$port/users" \
	-H 'Content-Type: application/json' \
...
echo Expected response: 200
curl -Lik "$ptcl://$host:$port/users?first_name=duke" \
	-b testcookie1

printf "\n=> TEST <=\n"
echo Send a GET request, authenticated as user1, with a query on last name.
echo Expected response: 200
curl -Lik "$ptcl://$host:$port/users?last_name=nuke" \
	-b testcookie1
